 ...
 * 
 * This middleware checks if the authenticated user's email has been verified.
 * It should be used after the protect() middleware.
 * 
 * Use this for routes that require users to have verified their email.
 * 
 * Example usage:
 * router.get('/sensitive-data', protect, requireVerified, sensitiveController);
 * 
 * @param {Object} req - Express request object
 * @param {Object} res - Express response object
 * @param {Function} next - Express next function
...
exports.requireVerified = async (req, res, next) => {
  try {
    ...
    // Query the database to check if user is verified
    // We do this fresh from the DB in case verification status changed
    ...
    if (!user.is_verified) {
      return next(new ErrorResponse('Please verify your email before accessing this route. Check your inbox for the verification link.', 403));
    }
